...
//----------------------------------------------------------------------------------------------------------
// 20120413 |  1.2.3   | Nino Liu   |  add frequency counter id in Setting file
//==========================================================================================================
...
namespace Dongle_Test_Suite_2._1
{
    public partial class MainForm : Form
    {
        ...
        public MainForm()
        {
            ...
            if(refradioattached && counterattached) UpdateOutputText("Welcome to the ThinkEco USB Test Module B. Please attach the dongle to be tested and scan its bottom housing MAC address label to begin.");
            if(!refradioattached) RunButtonEnabled(0);
        }
        ...
        public void MainProcess()      //THIS IS THE MAIN CODE THAT RUNS WHEN THE BUTTON IS CLICKED
        {
            ...
            try
            {
                ...
                if (parameters.testing && parameters.loading) finaltext = "Test and load with MAC address " + parameters.MAC + " have completed successfully in " + parameters.totaltesttime + " seconds.";
                else if (parameters.loading) finaltext = "Loading  with MAC address " + parameters.MAC + " has completed successfully in " + parameters.totaltesttime + " seconds.";
                else finaltext = "Testing  with MAC address " + parameters.MAC + " has completed successfully in " + parameters.totaltesttime + " seconds.";
                UpdateOutputText(finaltext + '\n' + '\n' + "Waiting for USB to be unplugged...");
                thisUSB.WaitForUnplug();
                UpdateOutputText(finaltext + '\n' + '\n' + "USB unplugged, ready for next test.");
                UpdateColorDisplay("white");
        ...
        public void ExceptionHandler(Exception ex, string category)
        {
            ...
            Finish();
            SelectScannerInputBox("");
            if (!ex.Message.Contains("Reference radio port could not be opened")) RunButtonEnabled(0);
            else RunButtonEnabled(1);
            ...
            else if (category == "red" && !ex.Message.Contains("already been successfully loaded"))
...
//            if (CounterID.Text.Length != null && CounterID.Text.Length <= 2 )
        ...
        private void CheckPreviouslySetTrims()
        {
            thisUSB.adjustBaudRate(115200);
            thisUSB.poll();
            Utils utils = new Utils();
            byte[] DongleHWInfoReq = { 0x02, 0x40, 0x04, 0x00, 0x44};
            UpdateOutputText("Reading trim values set by Dongle Test Module A...");
            thisUSB.WriteByte(DongleHWInfoReq);
            byte[] donglehwinforesponse = null;
            while (donglehwinforesponse == null)
            {
                System.Threading.Thread.Sleep(100);
                donglehwinforesponse = utils.AddtoReceiveList_ListenForNewFWResponse(thisUSB, parameters);
                //Utils.WaitForZTCResponse(thisUSB, parameters);
            }
            if (donglehwinforesponse.Length != 27) throw new Exception_Yellow("Bad response to hardware info request; previously set trim values could not be read.  Try again.");
            
            int coarsetrimread = donglehwinforesponse[24];
            int finetrimread = donglehwinforesponse[25];
            if (coarsetrimread == 0xFF || finetrimread == 0xFF) throw new Exception_Red("This dongle does not have trim values loaded.  Please send back to frequency counter station with Dongle Test Module A.");
            else if (coarsetrimread > 0x1F || finetrimread > 0x1F) throw new Exception_Red("This dongle was programmed with invalid trim values.  Please send back to frequency counter station with Dongle Test Module A.");

            
            parameters.coarsetrim_SET = donglehwinforesponse[24];
            parameters.finetrim_SET = donglehwinforesponse[25];
        }
